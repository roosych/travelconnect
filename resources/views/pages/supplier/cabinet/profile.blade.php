...
@php
    $__words = array_values(array_filter(explode(' ', trim($supplierName ?? ''))));
    $__ini   = count($__words) >= 2
        ? mb_strtoupper(mb_substr($__words[0], 0, 1) . mb_substr(end($__words), 0, 1))
        : mb_strtoupper(mb_substr($supplierName ?? '?', 0, 2));
@endphp
                ...
                <div class="d-flex flex-column flex-sm-row align-items-sm-center gap-6"
                    ...
                    <div class="position-relative">
                        <div id="avatar-preview" class="symbol symbol-125px symbol-circle">
                            @if($supplierAvatar)
                                <img id="avatar-img" src="{{ $supplierAvatar }}" alt="{{ __('suppliers.cabinet.profile.logo_alt') }}" class="object-fit-cover" />
                            @else
                    ...
                    <div class="flex-grow-1">
                        <div class="fw-bold fs-4 text-gray-800">{{ $supplierName ?? '—' }}</div>
...
<script>
    ...
    byId('profile-form').addEventListener('submit', async function (e) {
        ...
        try {
            ...
            const msg = ex?.errors ? Object.values(ex.errors).flat().join(' ') : (ex?.message ?? @json(__('suppliers.cabinet.profile.save_err')));
            err.textContent = msg;
            err.classList.remove('d-none');
        } finally {
    ...
    // ── Аватар поставщика ──────────────────────────────────────────────────
    (function () {
        const wrap = document.querySelector('[data-supplier-id]');
        if (!wrap) return;
        const supplierId = wrap.dataset.supplierId;
        ...
        const showImage = (url) => {
            preview.innerHTML = `<img id="avatar-img" src="${url}" alt="${@json(__('suppliers.cabinet.profile.logo_alt'))}" class="object-fit-cover" />`;
        ...
        fileInput.addEventListener('change', async function () {
            ...
            if (file.size > 2 * 1024 * 1024) {
                showToast(@json(__('suppliers.cabinet.profile.logo_too_big')), 'error');
                this.value = '';
                return;
            }
            ...
            try {
                ...
                if (res.ok && data.avatar_url) {
                    ...
                    showToast(@json(__('suppliers.cabinet.profile.logo_loaded')));
                } else {
                    showToast(data.message ?? @json(__('suppliers.cabinet.profile.logo_upload_err')), 'error');
                }
            } catch {
        ...
        btnRemove.addEventListener('click', async function () {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    showToast(@json(__('suppliers.cabinet.profile.logo_delete_err')), 'error');
                }
            } catch {
